function [Vrefl1,Vrefl2] = VtransTransfer(Omega,Ain,Bin,epsilon,ka_in,ka_out,ka_l,kb_in,kb_out,kb_l,Delta_a,Delta_b,Vin,Vout,varargin)
%
% Computes the 
...
%Computing the contributions to total noise sourcewise
    ...
    THEATA_in = LambdaMat*(Mout*inv(1i.*Omega.*eye(4)-Mc)*Min)*inv(LambdaMat); %Compute noise coupled from different ports and disturbance mechanisms
    THEATA_out = LambdaMat*(Mout*inv(1i.*Omega.*eye(4)-Mc)*Mout-eye(4))*inv(LambdaMat);
    THEATA_l = LambdaMat*Mout*inv(1i.*Omega.*eye(4)-Mc)*Ml*inv(LambdaMat);
    THEATA_Delta = LambdaMat*Mout*inv(1i.*Omega.*eye(4)-Mc)*XDelta;
    THEATA_epsilon = LambdaMat*Mout*inv(1i.*Omega.*eye(4)-Mc)*Xepsilon;
