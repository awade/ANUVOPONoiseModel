function THETA_epsil = THETA_epsilon(Omega,Ain,Bin,epsilon,ka_in,ka_out,ka_l,kb_in,kb_out,kb_l,Delta_a,Delta_b,varargin)
%
% Computes the 
% THETA_epsil Computes the THETA_epsilon Matrix from Chi_epsilon,
...
% 
 %Chi_epsilon = [conj(a_ss).*b_ss.*epsilon_delta;a_ss.*conj(b_ss).*conj(epsilon_delta);-0.5.*a_ss.^2.*conj(epsilon_delta);-0.5.*conj(a_ss).^2.*epsilon_delta]; %Fluctuation component of the coupling coefficient
...
% The individual mirror coupling rate matrixes
 ...
 Mepsilon = diag([epsilon_delta conj(epsilon_delta) epsilon_delta conj(epsilon_delta)]);
    THETA_epsil = LambdaMat*Min*inv(1i.*Omega.*eye(4)-Mc)*Mepsilon*inv(LambdaMat);
