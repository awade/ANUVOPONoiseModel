function [Vrefl1,Vrefl2] = VTheta(varargin) %VReflTheta(THETA_1,X_1,...Theta_n,X_n,Phi)
%
% Computes the 
...
%
% Need to take the X out of the Theta_Delta and Theta_epsilon functions
%
...
for ii = 1:floor(nargin/2) %Loop stepping through each pair of THETA and X terms
    Vout_Terms(:,ii) = abs((quadRotation(Phi)*varargin{ii*2-1})*varargin{ii*2}).^2; %Compute matrix with variance entries for each noise source
end
...
Vrefl2 = sum(Vout_Terms(2,:));%Select second quadrature of total output noise for the fundamental and sum accross all the noises computed above.
